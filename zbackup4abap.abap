...
FORM frm_get_tables .
  ...
  SELECT
    ...
    LEFT JOIN dd02t AS ds ON ds~tabname = dd~tabname
                         AND ds~ddlanguage = @sy-langu
    WHERE dd~tabname LIKE 'Z%'
      AND dd~tabclass <> 'CLUSTER'
      AND dd~tabclass <> 'POOL'
  ...
  DATA: lv_folder TYPE char2,
        lv_max    TYPE i.
  DATA: lt_table_lines TYPE TABLE OF string,
        lt_doman_lines TYPE TABLE OF string.
  DATA: lt_value TYPE TABLE OF string,
        lv_value TYPE string.
  DATA: lt_idomaindataa TYPE STANDARD TABLE OF dd07v WITH HEADER LINE.
  DATA: lt_idomaindatan TYPE STANDARD TABLE OF dd07v WITH HEADER LINE.
  ...
  LOOP AT lt_dd02l INTO DATA(ls_dd02l).
    ...
    " map 文件路径
    PERFORM frm_set_map_file USING lv_filename ls_dd02l-ddtext.
    ...
    IF sy-subrc = 0 AND ls_dd02v IS NOT INITIAL.
    ...
    lv_value = `<!DOCTYPE html><html lang="zh"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge">`.
    lv_value = lv_value && |<title>{ ls_dd02l-tabname }</title></head><body>|.
    " >> 字段填写
    lv_value = lv_value
            && `<table id='table' ><tr><th>` && `序号` && `</th>`
            && `<th>` && `字段名`   && `</th>`
            && `<th>` && `Key`      && `</th>`
            && `<th>` && `数据元素` && `</th>`
            && `<th>` && `数据域`   && `</th>`
    ...
    APPEND lv_value TO lt_table_lines.
    CLEAR lv_value.

    LOOP AT lt_dd03p INTO DATA(ls_dd03p).

      " >> 读取 doman
      APPEND |{ ls_dd03p-position ALPHA = OUT }| TO lt_value. " 序号
      APPEND |{ ls_dd03p-fieldname }|            TO lt_value. " 字段名
      APPEND |{ ls_dd03p-keyflag   }|            TO lt_value. " Key
      APPEND |{ ls_dd03p-rollname  }|            TO lt_value. " 数据元素
      APPEND |{ ls_dd03p-domname   }|            TO lt_value. " 数据域
      APPEND |{ ls_dd03p-datatype  }|            TO lt_value. " 类型
      APPEND |{ ls_dd03p-leng      }|            TO lt_value. " 长度
      APPEND |{ ls_dd03p-ddtext    }|            TO lt_value. " 描述

      CONCATENATE LINES OF lt_value INTO lv_value SEPARATED BY `</td><td>`.

      APPEND |<tr><td>{ lv_value }</td></tr></table>| TO lt_table_lines.
      CLEAR: lv_value, lt_value[].

      " >> DOMA 补充
      IF ls_dd03p-domname IS NOT INITIAL.
        CALL FUNCTION 'DD_DOMA_GET'
          EXPORTING
            domain_name   = ls_dd03p-domname
            get_state     = 'M  '
            withtext      = 'X'
          TABLES
            dd07v_tab_a   = lt_idomaindataa[]
            dd07v_tab_n   = lt_idomaindatan[]
          EXCEPTIONS
            illegal_value = 1
            op_failure    = 2.

        LOOP AT lt_idomaindataa.
          AT FIRST.
            PERFORM frm_add_doman_head USING lv_value ls_dd03p-domname.
            APPEND lv_value TO lt_doman_lines.
            CLEAR lv_value.
          ENDAT.

          APPEND |{ lt_idomaindataa-domname ALPHA = OUT }| TO lt_value. " 域名
          APPEND |{ lt_idomaindataa-domvalue_l }|          TO lt_value. " 域值
          APPEND |{ lt_idomaindataa-domvalue_h }|          TO lt_value. " 值描述
          APPEND |{ lt_idomaindataa-ddtext     }|          TO lt_value. " 域文本
        ENDLOOP.

        CONCATENATE LINES OF lt_value INTO lv_value SEPARATED BY `</td><td>`.

        APPEND |<tr><td>{ lv_value }</td></tr></table>| TO lt_doman_lines.
      ENDIF.
    ...
    CONCATENATE LINES OF lt_table_lines INTO lv_source SEPARATED BY gc_newline.
    CONCATENATE LINES OF lt_doman_lines INTO lv_value  SEPARATED BY gc_newline.
    lv_source = lv_source && lv_value && `</body></html>`.
    CLEAR: lt_table_lines[], lt_doman_lines[], lv_value.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form frm_add_doman_head
*&---------------------------------------------------------------------*
*&  头
*&---------------------------------------------------------------------*
FORM frm_add_doman_head  USING p_value p_field.
  p_value = |<table id='{ p_field }' ><tr>|
          && `<th>` && `域名`     && `</th>`
          && `<th>` && `域值`     && `</th>`
          && `<th>` && `值描述`   && `</th>`
          && `<th>` && `域文本`   && `</th>`.
